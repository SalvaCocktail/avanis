...
<aui:form action="<%= updateFormCalendarBookingURL %>" method="post" name="fm"
          onSubmit='<%= "event.preventDefault();" + liferayPortletResponse.getNamespace() + "updateCalendarBooking();" %>'>
    ...
    <div class="lfr-form-content">
        <clay:sheet>
            <liferay-ui:error exception="<%= CalendarBookingDurationException.class %>"
                              message="please-enter-a-start-date-that-comes-before-the-end-date"/>
            <liferay-ui:error exception="<%= CalendarBookingRecurrenceException.class %>"
                              message="the-last-repeating-date-should-come-after-the-event-start-date"/>
            ...
            <clay:sheet-section>
                <aui:input defaultLanguageId="<%= LocaleUtil.toLanguageId(themeDisplay.getSiteDefaultLocale()) %>"
                           name="title"/>

                <div class="<%= allDay ? "allday-class-active" : "" %>" id="<portlet:namespace />startDateContainer">
                    <aui:input ignoreRequestValue="<%= true %>" label="starts" name="startTime"
                               timeFormat="<%= timeFormat %>" value="<%= startTimeJCalendar %>"/>
                </div>

                <div class="<%= allDay ? "allday-class-active" : "" %>" id="<portlet:namespace />endDateContainer">
                    <aui:input ignoreRequestValue="<%= true %>" label="ends" name="endTime"
                               timeFormat="<%= timeFormat %>" value="<%= endTimeJCalendar %>"/>
                </div>
                ...
                <aui:field-wrapper cssClass="calendar-portlet-recurrence-container" inlineField="<%= true %>" label="">
                    ...
                    <a class="calendar-portlet-recurrence-summary" href="javascript:void(0);"
                       id="<portlet:namespace />summary"></a>
                </aui:field-wrapper>
                ...
                <%
                    ...
                    if (calendarBooking != null) {
                        ...
                        if (eventModalities != null && eventModalities.length > 0) {
                            eventModality = eventModalities[0];
                        }
                ...
                <aui:input type="text" name="event_organizer" label="Organizador" value="<%= eventOrganizer%>"/>
                <aui:input type="text" name="event_website" label="Web oficial del evento" value="<%= eventWebsite%>">
                    <aui:validator name="url"></aui:validator>
                </aui:input>
                <aui:input type="text" name="event_url" label="Url para redireccionar al detalle del evento"
                           value="<%= eventUrl%>">
                    <aui:validator name="required" errorMessage="La Url del evento es requerida"/>
                    <aui:validator name="max"
                                   errorMessage="La Longitud de la Url del evento no puede exceder los 300 caracteres">300</aui:validator>
                    <aui:validator name="custom"
                                   errorMessage="La url debe contener unicamente texto con espacios o guiones, ningun otro car&aacute;cter">
                        function (val, fieldNode, ruleValue) {
                        let pattern = /^[a-zA-Z0-9^-]{0,300}?$/;
                ...
                <aui:select label="Tipo de evento" name="event_type">
                    <aui:option value="all" selected='<%= "all".equals(eventType)%>'>Todos los tipos</aui:option>
                    <aui:option value="exhibition"
                                selected='<%= "exhibition".equals(eventType)%>'>Exhibici&oacute;n </aui:option>
                    <aui:option value="webinar" selected='<%= "webinar".equals(eventType)%>'>Webinar</aui:option>
                    <aui:option value="training"
                                selected='<%= "training".equals(eventType)%>'>Formaci&oacute;n</aui:option>
                    <aui:option value="congress" selected='<%= "congress".equals(eventType)%>'>Congreso</aui:option>
                    <aui:option value="fair" selected='<%= "fair".equals(eventType)%>'>Feria</aui:option>
                </aui:select>


                <aui:select label="Modalidad de evento" name="event_modality">
                    <aui:option value="all"
                                selected='<%= "all".equals(eventModality)%>'>Todas las modalidades</aui:option>
                    <aui:option value="online"
                                selected='<%= "online".equals(eventModality)%>'>En l&iacute;nea </aui:option>
                    <aui:option value="person" selected='<%= "person".equals(eventModality)%>'>Presencial</aui:option>
                    <aui:option value="hybrid"
                                selected='<%= "hybrid".equals(eventModality)%>'>H&iacute;brido</aui:option>
                </aui:select>

                <aui:select label="Valor de evento" name="event_value">
                    <aui:option value="undefined"
                                selected='<%= "undefined".equals(eventValue)%>'>Por confirmar</aui:option>
                    <aui:option value="free" selected='<%= "free".equals(eventValue)%>'>Gratuito</aui:option>
                    <aui:option value="paid" selected='<%= "paid".equals(eventValue)%>'>De pago</aui:option>
                    <aui:option value="past" selected='<%= "past".equals(eventValue)%>'>Pasado</aui:option>
                </aui:select>
            ...
            <clay:panel-group>
                ...
                >
                    <div class="panel-body">
                        <aui:select label="calendar" name="calendarId">
                            ...
                            <aui:option selected="<%= curCalendar.getCalendarId() == calendarId %>"
                                        value="<%= curCalendar.getCalendarId() %>"><%= HtmlUtil.escape(calendarName) %>
                            </aui:option>
                ...
                >
                    <div class="panel-body">
                        <c:if test="<%= invitable %>">
                            <aui:input inputCssClass="calendar-portlet-invite-resources-input" label=""
                                       name="inviteResource" placeholder="add-people-sites-rooms" type="text"/>
                        ...
                        >
                            ...
                            >
                                <label class="field-label">
                                    <liferay-ui:message key="pending[calendar]"/> (<span
                                        id="<portlet:namespace />pendingCounter"><%= pendingCalendarsJSONArray.length() %></span>)
                                </label>

                                <div class="calendar-portlet-calendar-list"
                                     id="<portlet:namespace />calendarListPending"></div>
                            </clay:col>
                            ...
                            >
                                <label class="field-label">
                                    <liferay-ui:message key="accepted"/> (<span
                                        id="<portlet:namespace />acceptedCounter"><%= acceptedCalendarsJSONArray.length() %></span>)
                                </label>

                                <div class="calendar-portlet-calendar-list"
                                     id="<portlet:namespace />calendarListAccepted"></div>
                            </clay:col>
                            ...
                            >
                                <label class="field-label">
                                    <liferay-ui:message key="declined"/> (<span
                                        id="<portlet:namespace />declinedCounter"><%= declinedCalendarsJSONArray.length() %></span>)
                                </label>

                                <div class="calendar-portlet-calendar-list"
                                     id="<portlet:namespace />calendarListDeclined"></div>
                            </clay:col>
                            ...
                            <c:if test="<%= calendarBooking != null %>">
                                ...
                                >
                                    <label class="field-label">
                                        <liferay-ui:message key="maybe"/> (<span
                                            id="<portlet:namespace />maybeCounter"><%= maybeCalendarsJSONArray.length() %></span>)
                                    </label>

                                    <div class="calendar-portlet-calendar-list"
                                         id="<portlet:namespace />calendarListMaybe"></div>
                                </clay:col>
                            </c:if>
                            ...
                            >
                                <div class="calendar-portlet-list-header toggler-header-collapsed"
                                     id="<portlet:namespace />checkAvailability">
                                    <span class="calendar-portlet-list-arrow"></span>

                                    <span class="calendar-portlet-list-text"><liferay-ui:message
                                            key="resources-availability"/></span>
                                </div>
                                ...
                                <div class="calendar-portlet-availability">
                                    <div class="toggler-content-collapsed" id="<portlet:namespace />schedulerContainer">
                                        ...
                                        <liferay-util:include page="/scheduler.jsp" servletContext="<%= application %>">
                                            ...
                                            <liferay-util:param name="filterCalendarBookings"
                                                                value='<%= liferayPortletResponse.getNamespace() + "filterCalendarBookings" %>'/>
                                            <liferay-util:param name="hideAgendaView"
                                                                value="<%= Boolean.TRUE.toString() %>"/>
                                            <liferay-util:param name="hideMonthView"
                                                                value="<%= Boolean.TRUE.toString() %>"/>
                                            <liferay-util:param name="preventPersistence"
                                                                value="<%= Boolean.TRUE.toString() %>"/>
                ...
                <c:if test="<%= calendarBookingId > 0 %>">
                    <clay:panel displayTitle='<%= LanguageUtil.get(request, "related-assets") %>'>
                        <div class="panel-body">
                            <%@ include file="/custom_input_asset_link.jspf" %>
                        </div>
                    </clay:panel>
                    <clay:panel displayTitle='Imagen'>
                        <div class="panel-body">
                            <%@ include file="/image_form.jspf" %>
                        </div>
                    </clay:panel>

                    <clay:panel displayTitle='Protagonistas'>
                        <div class="panel-body">
                            <%@ include file="/protagonist_form.jspf" %>
                        </div>
                    </clay:panel>

                    <clay:panel displayTitle='Patrocinadores'>
                        <div class="panel-body">
                            <%@ include file="/sponsor_form.jspf" %>
                        </div>
                    </clay:panel>
    ...
    <aui:button-row cssClass="d-block">
        <div class="alert alert-info <%= (hasWorkflowDefinitionLink && approved) ? StringPool.BLANK : "hide" %>"
             id="<portlet:namespace />approvalProcessAlert">
            <liferay-ui:message
                    arguments="<%= ResourceActionsUtil.getModelResource(locale, CalendarBooking.class.getName()) %>"
...
<aui:script
    ...
    <c:if test="<%= invitable %>">
        ...
        <liferay-portlet:resourceURL copyCurrentRenderParameters="<%= false %>" id="calendarResources"
                                     var="calendarResourcesURL"/>
