...
<c:if test="<%= CalendarPermission.contains(themeDisplay.getPermissionChecker(), calendar, CalendarActionKeys.VIEW_BOOKING_DETAILS) %>">
	...
	<div>
		<liferay-portlet:runtime portletName="avanis_seguir_eventos_AvanisSeguirEventosPortlet"
								 queryString='<%="calendarBookingId=" + calendarBooking.getCalendarBookingId()  %>'
								 instanceId="<%= String.valueOf(calendarBooking.getCalendarBookingId()) %>"
		/>
	</div>
</c:if>