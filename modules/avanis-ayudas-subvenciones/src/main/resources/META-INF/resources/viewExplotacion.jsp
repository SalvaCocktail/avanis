...
<c:choose>
    <c:when test="${not empty articlesData}">
    ...
    <c:otherwise>
        <h4 class="av-widget--title h4 separador empty-state-ayudas">Ayudas y subvenciones</h4>
    <div class="profile__non-data av-lm-cl__favorites-empty-state">
        <img class="candado" src="<%=request.getContextPath()%>/images/fence.svg" alt="icono-candado" />
        Aún no hay ayudas que afecten a esta explotación, ¿Quieres consultar otras ayudas que estén activas?
        <a href="/es/ayudas" class="ver-todas av-event-header-link-span">Ver todas las ayudas</a>
    </div>
        </c:otherwise>
</c:choose>
