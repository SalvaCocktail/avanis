...
@Component(
    ...
    property = {
        ...
        "mvc.command.name="+ AvanisBuscadorPortletKeys.AVANISBUSCADORNOTICIAS_FILTRARNOTICIAS
    },
    service = MVCResourceCommand.class,
    configurationPolicy = ConfigurationPolicy.OPTIONAL
)
public class FiltrarNoticias extends BaseMVCResourceCommand {
    ...
    protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws Exception {
        ...
        _log.info(" Filtros seleccionados: " + selectedFilters);
        _log.info(" Filtros de fecha aplicados: " + diasFiltro);
        _log.info(" Filtros de tipo aplicados: " + tipos);
        _log.info(" Filtros de categoría aplicados: " + categoryIds);

        for (String filtro : selectedFilters.split(",")) {
            _log.info(" Filtro recibido: " + filtro);

            if (filtro.startsWith("tipo_")) {
                String tipo = filtro.replace("tipo_", "");
                tipos.add(tipo);
            }
        ...
        List<Noticia> noticiasFiltradas = _noticiasUtils.getNoticiasPorCategoriasYBusqueda(
                themeDisplay, categoryIds, tipos, diasFiltro, searchQuery
        );
