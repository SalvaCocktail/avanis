...
import avanis.comunidad.portlet.util.MBRequestUtil;
...
public class DefaultMBListDisplayContext implements MBListDisplayContext {
	...
	public DefaultMBListDisplayContext(
		...
		if (mvcRenderCommandName.equals("/avanis_message_boards/view_my_posts")) {
			showMyPosts = true;
		}
	...
	public int getCategoryEntriesDelta() {
		...
		return GetterUtil.getInteger(
			portalPreferences.getValue(
					AvanisComunidadPortletKeys.AVANISCOMUNIDAD, "categoryEntriesDelta"),
	...
	public int getThreadEntriesDelta() {
		...
		return GetterUtil.getInteger(
			portalPreferences.getValue(
					AvanisComunidadPortletKeys.AVANISCOMUNIDAD, "threadEntriesDelta"),
			SearchContainer.DEFAULT_DELTA);
	}
	...
	private boolean _isShowRecentPosts(String mvcRenderCommandName) {
		if (mvcRenderCommandName.equals("/avanis_message_boards/view_recent_posts")) {
			return true;
		}
	...
	private boolean _isShowSearch(String mvcRenderCommandName) {
		if (Validator.isNotNull(
				ParamUtil.getString(_httpServletRequest, "keywords")) ||
			mvcRenderCommandName.equals("/avanis_message_boards/search")) {
