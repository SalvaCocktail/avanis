package avanis.eventos.minicalendar.portlet;

import avanis.calendarbooking.sb.model.Protagonist;
import avanis.calendarbooking.sb.service.ProtagonistLocalServiceUtil;

import avanis.eventos.minicalendar.util.CalendarResourceUtil;
import avanis.eventos.minicalendar.util.CalendarUtil;
import avanis.eventos.minicalendar.constants.AvanisEventosMiniCalendarKeys;
import avanis.eventos.minicalendar.display.context.CalendarDisplayContext;
...
import static avanis.eventos.minicalendar.util.CalendarResourceUtil.getScopeGroupCalendarResource;
import static avanis.eventos.minicalendar.util.CalendarResourceUtil.getUserCalendarResource;
...
@Component(
        ...
        property = {
                ...
                "javax.portlet.display-name=Avanis Eventos Minicalendar",
                "javax.portlet.init-param.template-path=/",
                "javax.portlet.init-param.view-template=/view.jsp",
                "javax.portlet.name=" + AvanisEventosMiniCalendarKeys.AVANISEVENTOSMINICALENDAR,
...
public class AvanisEventosMiniCalendar extends MVCPortlet {
    ...
    public void serveResource(
        ...
        try {
            ...
            if (resourceID.equals("calendar")) {
                ...
                _serveCalendarBookingInvitees(resourceRequest, resourceResponse);
            ...
            else if (resourceID.equals("calendarBookings2")) {
            ...
            else if (resourceID.equals("mesAnio")) {

                int year = Integer.parseInt(resourceRequest.getParameter("year"));
                int month = Integer.parseInt(resourceRequest.getParameter("month"));
                int lastDay = Integer.parseInt(resourceRequest.getParameter("daysInMonth"));
                int startDay = 1;

                // Crear LocalDate para el primer día del mes
                LocalDate startDate = LocalDate.of(year, month, startDay);
                // Crear LocalDate para el último día del mes
                LocalDate endDate = LocalDate.of(year, month, lastDay);

                // Convertir las fechas a milisegundos
                ZonedDateTime startZonedDateTime = startDate.atStartOfDay(ZoneId.systemDefault());
                long startTimeMillis = startZonedDateTime.toInstant().toEpochMilli();
                ZonedDateTime endZonedDateTime = endDate.atStartOfDay(ZoneId.systemDefault());
                long endTimeMillis = endZonedDateTime.toInstant().toEpochMilli();
                List<CalendarBooking> listaDeCalendario= getBookingList(resourceRequest,resourceResponse,startTimeMillis,endTimeMillis);

                //List<CalendarBooking> calendarBookings = BookingsHandlerResource.getBookingList((RenderRequest) resourceRequest, (RenderResponse) resourceResponse);
                for (CalendarBooking booking : listaDeCalendario) {
                    Instant instant = Instant.ofEpochMilli(booking.getStartTime());
                    ...
                    instant = Instant.ofEpochMilli(booking.getEndTime());
                    localDate = instant.atZone(ZoneId.systemDefault()).toLocalDate();

                }

                // Llamar al método que sirve el recurso
                _serveCalendarBookings(resourceRequest, resourceResponse);
            }
                ...
                _serveHasExclusiveCalendarBooking(resourceRequest, resourceResponse);
    ...
    public static List<CalendarBooking> getBookingList(ResourceRequest renderRequest, ResourceResponse renderResponse, Long startTime, Long endTime) {
        ...
        try {
            CalendarResource groupCalendarResource = getScopeGroupCalendarResource(renderRequest, themeDisplay.getScopeGroupId());
            ...
            // Obtener el mes visible
            java.util.Calendar visibleCal = java.util.Calendar.getInstance();
            visibleCal.setTimeInMillis(startTime);
            int visibleMonth = visibleCal.get(java.util.Calendar.MONTH);
            int visibleYear = visibleCal.get(java.util.Calendar.YEAR);

            // Último día del mes visible
            int lastDay = visibleCal.getActualMaximum(java.util.Calendar.DAY_OF_MONTH);

            Map<Integer, Integer> dayEventCount = new HashMap<>();

            // Inicializar el mapa para cada día del mes visible
            for (int day = 1; day <= lastDay; day++) {
                dayEventCount.put(day, 0);
            }

            // Recorrer cada evento y contar solo los días que caen en el mes visible
            for (CalendarBooking booking : calendarBookings) {
                java.util.Calendar startCal = java.util.Calendar.getInstance();
                startCal.setTimeInMillis(booking.getStartTime());

                java.util.Calendar endCal = java.util.Calendar.getInstance();
                endCal.setTimeInMillis(booking.getEndTime());

                // Ajustar la fecha de inicio si el evento comienza antes del mes visible
                if (startCal.get(java.util.Calendar.YEAR) < visibleYear || (startCal.get(java.util.Calendar.YEAR) == visibleYear && startCal.get(java.util.Calendar.MONTH) < visibleMonth)) {
                    startCal.set(java.util.Calendar.YEAR, visibleYear);
                    startCal.set(java.util.Calendar.MONTH, visibleMonth);
                    startCal.set(java.util.Calendar.DAY_OF_MONTH, 1); // Primer día del mes visible
                }

                // Ajustar la fecha de fin si el evento termina después del mes visible
                if (endCal.get(java.util.Calendar.YEAR) > visibleYear || (endCal.get(java.util.Calendar.YEAR) == visibleYear && endCal.get(java.util.Calendar.MONTH) > visibleMonth)) {
                    endCal.set(java.util.Calendar.YEAR, visibleYear);
                    endCal.set(java.util.Calendar.MONTH, visibleMonth);
                    endCal.set(java.util.Calendar.DAY_OF_MONTH, lastDay); // Último día del mes visible
                }

                // Obtener el día de inicio y fin del evento dentro del mes visible
                int eventStartDay = startCal.get(java.util.Calendar.DAY_OF_MONTH);
                int eventEndDay = endCal.get(java.util.Calendar.DAY_OF_MONTH);

                // Contar los días del evento dentro del mes visible
                for (int day = eventStartDay; day <= eventEndDay; day++) {
                    dayEventCount.put(day, dayEventCount.get(day) + 1);
                }
            }

            // Convertir el mapa a JSONObject
            JSONObject jsonObject = JSONFactoryUtil.createJSONObject();
            for (Map.Entry<Integer, Integer> entry : dayEventCount.entrySet()) {
    ...
    private void removeProtagonist(ResourceRequest resourceRequest, ResourceResponse resourceResponse) {
        ...
        //TODO: Borrar la relación también, auqnue quizás lo mejor sea eliminarla
    ...
    private void addProtagonist(ResourceRequest resourceRequest, ResourceResponse resourceResponse) {
        ...
        try {
            if (fileBytes != null) {
                ...
                Folder protagonistFolder = getOrCreateFolder(repositoryId, eventFolder.getFolderId(), "protagonists", userId);
            ...
            //TODO: El portraitID hay que ponerle un stringURL
    ...
    private void updateRelatedAssets(CalendarBooking calendarBooking, PortletRequest portletRequest, ThemeDisplay themeDisplay) throws IOException, PortalException {
    ...
    private void _getCalendar(PortletRequest portletRequest) throws Exception {
        ...
        portletRequest.setAttribute(AvanisEventosMiniCalendarKeys.CALENDAR, _calendarService.getCalendar(calendarId));
    }

    private void _getCalendarBooking(PortletRequest portletRequest)
            throws Exception {

        if (portletRequest.getAttribute(AvanisEventosMiniCalendarKeys.CALENDAR_BOOKING) !=
                null) {
        ...
        portletRequest.setAttribute(
                AvanisEventosMiniCalendarKeys.CALENDAR_BOOKING,
    ...
    private void _getCalendarResource(PortletRequest portletRequest)
        ...
        portletRequest.setAttribute(
                AvanisEventosMiniCalendarKeys.CALENDAR_RESOURCE, calendarResource);
    }
...
//					UserFirstNameComparator.getInstance(false));
    ...
    private void _setRenderRequestAttributes(
        ...
        renderRequest.setAttribute(
                AvanisEventosMiniCalendarKeys.CALENDAR_DISPLAY_CONTEXT, calendarDisplayContext);
    }
    ...
    private static Log _log = LogFactoryUtil.getLog(AvanisEventosMiniCalendar.class);
